...
class CalibrationWidget(QWidget):
    def __init__(self):
        ...
        allCalBtn.clicked.connect(self.zeroAll)
        calibrationPanelLayout.addWidget(allCalBtn, 0, 1, 2, 1)

        # individual calibration buttons
        thrustZeroBtn = QPushButton("Thrust")
        thrustZeroBtn.clicked.connect(board.zeroCell1)
        torqueZeroBtn = QPushButton("Torque")
        torqueZeroBtn.clicked.connect(board.zeroCell2)
        currentZeroBtn = QPushButton("Current")
        currentZeroBtn.clicked.connect(board.zeroCurrent)
        voltageZeroBtn = QPushButton("Voltage")
        voltageZeroBtn.clicked.connect(board.zeroVoltage)
        calibrationPanelLayout.addWidget(thrustZeroBtn, 0, 2)
        calibrationPanelLayout.addWidget(torqueZeroBtn, 0, 3)
        calibrationPanelLayout.addWidget(currentZeroBtn, 0, 4)
        calibrationPanelLayout.addWidget(voltageZeroBtn, 0, 5)
